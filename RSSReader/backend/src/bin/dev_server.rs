...
use rssreader_backend::reader::{original_page_fallback_html, render_original_page};
...
fn handle_connection(mut stream: TcpStream) {
    ...
    match (method, path) {
        ...
        ("GET", path) if path.starts_with("/api/render") => {
            ...
            match render_original_page(&target_url) {
                Ok(result) => write_proxied_html(&mut stream, &result.html),
                Err(message) => write_proxied_html(
                    &mut stream,
                    &original_page_fallback_html(&target_url, &message),
                ),
            }
        }
